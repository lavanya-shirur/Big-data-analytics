...
def getInvertedIndex(inputFile, topNItems, wordRemoveList=[], outputDir="InvertedIndex"):
    text = sc.wholeTextFiles(inputFile)
    ...
    wordCountPerFile.saveAsTextFile(outputDir)
...
if __name__ == '__main__':
        ...
        inputFile = "/cosc6339_hw2/gutenberg-500"
        outputDir = "/bigd27/Inverted_IndexMediumdatasetFinal222"

        t1=time.time()
        print "Load Top 1000 words from", topWords_inputFile
        topNItems = loadTopNWordList(topWords_inputFile)

        print "Create Inverted Index and save to", outputDir
        getInvertedIndex(inputFile, topNItems, stop_words, outputDir)
        t2=time.time()
